package com.meishi.front.ext.email;
...
public class SendEmail {
	...
	static final Log logger = LogFactory.getLog(SendEmail.class);
	...
	public SendEmail(String mail_host) {
		this.mail_host = mail_host;
	}

	public SendEmail(String mail_host, boolean auth, String account,
			String password) {
		this(mail_host, 25, EncryptionTypes.Default.ordinal(), auth, account,
				password);
	}

	public SendEmail(String mail_host, int mail_port, int encryptionType,
	...
	public void sendEmail(String senderAddress, String senderName,
			List recipients, String sub, String msg) throws SendFailedException {
	 ...
	 */
	public void sendEmail(String senderAddress, String senderName,
			List recipients, String sub, String msg, Collection attachments)
			throws SendFailedException {

		logger.debug("email subject: " + sub + " mail_port: " + this.mail_port
	...
	private Properties getProperties() {
		...
		if (defaultEncryptionType == EncryptionTypes.TLS.ordinal()) {
			props.put("email.smtp.auth", String.valueOf(this.auth));
			props.put("email.smtp.starttls.enable", "true");
		} else if (defaultEncryptionType == EncryptionTypes.SSL.ordinal()) {
			props.put("email.smtp.host", this.mail_host);
			props.put("email.smtp.socketFactory.port", this.mail_port);
			props.put("email.smtp.socketFactory.class",
					"javax.net.ssl.SSLSocketFactory");
			props.put("email.smtp.debug", "true");
			props.put("email.smtp.auth", String.valueOf(this.auth));
			props.put("email.smtp.port", this.mail_port);
		} else {
			props.put("email.smtp.host", this.mail_host);
			props.put("email.smtp.auth", String.valueOf(this.auth));
		}
