...
public class ThreadSysLog {
	...
	private static Queue<PtSyslog> queue = new ConcurrentLinkedQueue<PtSyslog>(); //	此队列按照 FIFO（先进先出）原则对元素进行排序
	
	/**
	 * 向队列中增加Syslog对象，基于LinkedBlockingQueue
	 *public static void add(PtSyslog sysLog){
		try {
	 ...
	 * @return
	 */
	public static PtSyslog getSyslog(){
		synchronized(queue) {
			if(queue.isEmpty()){
				return null;
			}else{
