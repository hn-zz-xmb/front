...
public class CouponNotifyController extends BaseController {

	private static Logger log = Logger.getLogger(CouponNotifyController.class);
	...
	public void notifyPay() throws IOException {
		...
		if (AlipayNotify.verify(params)) {// 验证成功
			...
			if (trade_status.equals("TRADE_FINISHED")) {
				...
				// 如果有做过处理，不执行商户的业务程序
				CouponOrder orders = CouponOrder.dao.findByNo(out_trade_no);
				// 判断该笔订单是否在商户网站中已经做过处理
				CouponOrder.dao.afterPay(orders);
				// 注意：
				// 该种交易状态只在两种情况下出现
				// 1、开通了普通即时到账，买家付款成功后。
